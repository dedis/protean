...
func init() {
	onet.GlobalProtocolRegister(NameGetState, NewGetState)
}
...
func NewGetState(n *onet.TreeNodeInstance) (onet.ProtocolInstance, error) {
	p := &GetState{
		TreeNodeInstance: n,
...
func (p *GetState) execute(r StructGSRequest) error {
	defer p.Done()
	if p.Verify != nil {
...
func (p *GetState) executeReply(r StructGSResponse) error {
...
func (p *GetState) makeResponse(data []byte) (*GSResponse, error) {
...
func (p *GetState) finish(result bool) {
	p.timeout.Stop()
	select {
